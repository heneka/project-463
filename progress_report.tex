...
For the next phase, my technical direction involves diving deeper into the LightGCN architecture. Since LightGCN currently performs the best, I want to investigate hyperparameter optimization, specifically tuning the number of message-passing layers and the message dropout. I am also considering adding a margin-based BPR (Bayesian Personalized Ranking) loss explicitly to see if ranking metrics improve further, rather than basic binary cross entropy. If time allows, I might introduce user demographics or movie genres as side features.
