...
def load_movie_metadata(dataset):
    ...
    with open(movie_path, 'r', encoding='latin-1') as movie_file:
        for line in movie_file:
            ...
            movie_lookup[int(movie_id_str) - 1] = {
                'title': title,
                'genres': genres,
            }
    return movie_lookup
...
def pick_sample_user(test_data, num_users):
    positive_users = test_data.edge_label_index[0][test_data.edge_label == 1]
...
def main():
    ...
    user_embedding, recommendations = recommend_movies_for_user(
        model=model,
    ...
    positive_movies = get_positive_matches(
    ...
    for item in recommendations:
        ...
        report_lines.append(
            f"  - movie_index={item['movie_index']:<5} movie_node_id={item['movie_node_id']:<5} score={item['score']:<8} {format_movie(movie_lookup, item['movie_index'])} {match_flag}"
        )
    report_lines.append('')
