...
def main():
    ...
    models = {'LightGCN': LightGCNModel(num_nodes=num_nodes, embedding_dim=64, num_layers=3), 'GAT': GATModel(num_nodes=num_nodes, embedding_dim=64, num_layers=2), 'GraphSAGE': GraphSAGEModel(num_nodes=num_nodes, embedding_dim=64, num_layers=2), 'NCF': NCFModel(num_nodes=num_nodes, embedding_dim=64)}
    ...
    for (name, model) in models.items():
        ...
        (history, test_metrics) = train(model=model, train_data=train_data, val_data=val_data, test_data=test_data, num_nodes=num_nodes, num_users=num_u, num_movies=num_m, epochs=20, lr=0.005, device=device)
